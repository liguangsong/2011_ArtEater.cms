    ...
    <Row class="table-wrap">
      <Table :loading="loading" :columns="columns" :data="data">
        ...
        <template slot-scope="{ row }" slot="courseIds">
          <strong v-for="(_sub, _idx) in row.courseIds">{{
            getCourseName(_sub) + (_idx == row.courseIds.length - 1 ? "" : "；")
          }}</strong>
        </template>
    ...
    >
      ...
      >
        ...
        <FormItem label="视频封面图" prop="coverUrl">
          ...
          <myUpload
            @complate="handleUploadComplate1"
            tips="（推荐尺寸750*592）"
          ></myUpload>
        </FormItem>
    ...
    >
      <Form :model="form" label-position="right" :label-width="100" ref="form">
        ...
        <FormItem label="套课" prop="courseIds">
          ...
          <span v-for="(_sub, _idx) in form.courseIds">{{
            getCourseName(_sub) +
              (_idx == form.courseIds.length - 1 ? "" : "；")
          }}</span>
        </FormItem>
        <!-- <FormItem label="链接">
          {{form.href}}
        ...
        courseIds: [
          {
            ...
            message: "请选择套课",
            trigger: "change",
            validator: verification.validateArrayIsNull
          }
        ]
        ...
        video.addEventListener(
          ...
          function() {
            setTimeout(function() {
              ...
              var parseFile = new self.ParseServer.File("cover.jpg", {
                base64: data
              });
