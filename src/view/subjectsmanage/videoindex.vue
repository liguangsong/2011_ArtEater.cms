<template>
  <div class="container-wrap">
    <div class="header-wrap clear-fix" style="display:flex">
        <div class="search-wrap clear-fix" style="flex:1">
            <div class="search-keyword" style="width:260px">
                ...
                <Input v-model="search_keyword" size="large" placeholder="请输入ID/标题" style="width:200px" />
            </div>
            <div class="search-keyword" style="width:310px">
                <span style="width:100px;padding:0 5px">推荐套课名称 </span>
                <Input v-model="search_courseName" size="large" placeholder="请输入推荐套课名称" style="width:200px" />
            </div>
    ...
    <Row class="table-wrap">
        <Table :loading="loading" :columns="columns" :data="data">
            <template slot-scope="{ row }" slot="headImg">
                <div style="margin:5px 0"><img v-if="row.headImg" :src="row.headImg" @click="handleShowImg(row)" width="84" height="70"></div>
            </template>
    ...
    <Modal v-model="show_window" :loading="editLoading" :title="window_title" width="600" @on-ok="add_video">
      <Form v-if="show_window" :model="form" label-position="right" :label-width="100" ref="form" :rules="ruleValidate">
        <FormItem label="图片" prop="headImg">
            <div>
                <img v-if="form.headImg" :src="form.headImg" width="375" height="342"/>
            </div>
            <myUpload @complate="handleUploadComplate" tips="（推荐尺寸434*342px）" accept="image/*"></myUpload>
        </FormItem>
        <FormItem label="标题" prop="title">
          <Input v-model="form.title" placeholder="请输入标题"  style="width:400px"></Input>
        </FormItem>
        <FormItem label="推荐套课" prop="courseId">
          <selectTree v-if="show_window" id="mySelectTree" @change="handleChangeCouse" v-model="form.courseId" :treeData="courseTreeData" style="width:400px"></selectTree>
        </FormItem>
        <FormItem label="推荐课程" prop="videoId">
          <Select ref="myVideo" v-model="form.videoId" style="width:400px" not-found-text="暂无课程" :clearable="true">
            <Option v-for="video in videoList" :value="video.objectId" :key="video.objectId">{{ video.title }}</Option>
          </Select>
        </FormItem>
        <FormItem label="链接">
    ...
    <Modal v-model="isShowDetail" title="查看详情" mask scrollable width="600">
      <Form v-if="isShowDetail" :model="form" label-position="right" :label-width="100" ref="form">
      <FormItem label="图片" prop="headImg">
            <div>
                <img v-if="form.headImg" :src="form.headImg" width="375" height="342"/>
            </div>
...
export default {
  ...
  data () {
    return {
        ...
        ruleValidate: {
            title:  [{ required: true, message: '请输入推荐标题', trigger: 'blur' },{ required: true, message: '请输入推荐标题', trigger: 'change' }],
            headImg: [{required: true, message: "请上传图片", trigger: "change", validator: verification.validateIsNull}],
