<template>
  <div class="container-wrap">
    <div class="header-wrap clear-fix" style="display:flex">
        <div class="search-wrap clear-fix" style="flex:1">
            <div class="search-keyword" style="width:260px">
                ...
                <Input v-model="search_keyword" size="large" placeholder="请输入轮播图名称" style="width:200px" />
            </div>
        ...
        <div class="operation-wrap" style="width:175px">
            <Button style="margin-right:10px" type="primary" @click="handleAddBanner">添加</Button>
        </div>
    </div>
    ...
    <Modal v-model="show_window" :loading="editLoading" :title="window_title" mask scrollable width="600" @on-ok="add_Banner">
      <Form v-if="show_window" :model="form" label-position="right" :label-width="100" ref="form" :rules="ruleValidate">
        ...
        <FormItem label="轮播图" prop="img">
            ...
            <myUpload @complate="handleUploadComplate" tips="（推荐尺寸690*320px）"></myUpload>
        </FormItem>
        ...
        <FormItem label="是否上架" prop="state">
          <i-switch v-model="form.state" :true-value="1" :false-value="0"  size="large">
            <span slot="open">上架</span>
            <span slot="close">下架</span>
          </i-switch>
        </FormItem>
        <FormItem label="备注" prop="remark">
          <Input type="textarea" :rows="4" v-model="form.remark" placeholder="请输入备注"></Input>
        </FormItem>
      </Form>
    </Modal>
    <Modal width="740" title="查看大图" v-model="isShowImg">
        <div style="margin:5px 0;text-align:center"><img :src="currImg" width="690" height="320"></div>
    </Modal>
    <Modal v-model="isShowDetail" title="查看轮播图详情" mask scrollable width="600">
...
export default {
  ...
  data () {
    return {
        ...
        ruleValidate: {
            title: [{required: true, message: "请输入轮播图名称", trigger: "blur", validator: verification.validateIsNull}],
            img: [{required: true, message: "请上传轮播图", trigger: "blur", validator: verification.validateIsNull}],
